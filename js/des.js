...
(function() {
  ...
  */
  ...
  TRIPLE_SUBSET = function() {
    ...
    o.sync = function(start, T2, start2, common, callback) {
      var I, J, add_transition, e1, e2, has_callback, i, j, map, map_n, o2, p1, p2, q, q1, q2, stack, t1, t2, _i, _j, _k, _len, _len1, _len2;
      has_callback = typeof callback === 'function';
      o2 = T2.transitions;
      map = [start, start2];
      map_n = 1 | 0;
  ...
  make_sync = function(m1, m2) {
    var M, T, common;
    T = create_general_set(T_CONFIG);
    M = DES.make_module_from_T(T, 'sync(' + m.name + ')');
    M.X.start = 0;
    common = intersection(m1.common, m2.common);
    m1.T.transitions.sync(m1.X.start, m2.T, m2.X.start, common, function(q, e, p) {
      T.transitions.set(T.add(), q, e, p);
      if (q >= M.X.size()) {
        q = M.X.add();
      }
  ...
  (function() {
    ...
    while (i-- > 0) {
      ...
      _results.push(m.C.common = m.common);
    }
    return _results;
  })();

  (function() {
    var events, f, i, j, n, nf, s;
    i = 0;
    m = DES.modules[i];
    nf = make_NF_module(m);
    n = make_N_module(nf);
