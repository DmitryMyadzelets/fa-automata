...
(function() {
  'use strict';
  var friction, getLinkCurve, graph, height, is_linked, link_charge, link_charge_distance, link_distance, link_gravity, makeLink, node_radius, set_links, svg_container_id, update_SVG, update_svg_size, vec, width;
  ...
  update_SVG = function() {
    ...
    force.charge(link_charge).chargeDistance(link_charge_distance).gravity(link_gravity).friction(friction).size([width, height]).nodes(graph.nodes).links(graph.links).start();
    ...
    link = svg.selectAll('.link').data(graph.links).enter().append('path').attr("style", "fill: none; stroke: #000000");
  ...
  set_links = function() {
    var index, j, k, link, m1, m2, node, o, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2;
    ...
    for (j = _i = 0, _len = _ref.length; _i < _len; j = ++_i) {
      ...
      for (k = _j = 0, _len1 = _ref1.length; _j < _len1; k = ++_j) {
        ...
        if (!DES.get_common_events(m1, m2).length) {
          continue;
        }
        _ref2 = graph.links;
        for (index = _k = 0, _len2 = _ref2.length; _k < _len2; index = ++_k) {
          ...
          if ((link.source === k) && (link.target === j)) {
            break;
          }
        }
